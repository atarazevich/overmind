...
- **Hooks are fail-safe.** Always exit 0, never print to stdout (a hook's JSON output reaches the session), and do the network call detached under its own alarm. A broken Overmind must be invisible to Claude Code. Test every hook script by piping a sample payload before wiring it.
- **Hooks bound their work by counts, never by a clock.** Every entry is wired `"async": true`, so Claude Code neither waits for the hook nor enforces its `timeout` (code.claude.com/docs/en/hooks, "Run hooks in the background"). No latency is at stake, and nothing outside the hook stops one that runs away. What the bounds protect is the machine: one interpreter per event, across every session he runs at once. So each read is capped in bytes and lines and each parse in size, with the same answer at any load. A clock would make what a hook sees depend on how busy the machine is, and the machine is busiest exactly when the most hooks fire.
...
| `overmind/transcript.py` | The bounded tail of a session transcript: the turn's depth and tool calls, and whether the owner interrupted it |
| `overmind/rules.py` | The detectors that are deterministic, so they cost nothing and are never asked of a model |
