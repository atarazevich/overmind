...
- Stop: jumped (he asked a question; the turn acted) — the only conduct class that survived the truncation-matched control, at +38 pp. Beside it, in code and free: wrong_room (paths the turn wrote against paths his message named, +19 pp as a rule), and the fact `wants_you` (the last paragraph of the reply asks him something; 0.89 accurate held out, #17, an optimistic figure: the labels are the rule author's, not his).
- UserPromptSubmit: intervention (Nudge or Correction) — asked only when the transcript tail says the previous turn ended in an interruption. `interrupted`, `depth` and `tool_calls` are recorded as facts either way. A prompt that is a background agent's `<task-notification>` is not his and writes no line (#17).
